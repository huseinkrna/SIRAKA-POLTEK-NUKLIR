// src/components/simulator/FullChart.jsx
...
export default function FullChart({ historyData = [], currentData }) {
  // State untuk periode tetap dipertahankan, tapi diisi via useMemo + useEffect terpisah
  const [periode, setPeriode] = useState('∞');

  // Logic untuk sliding window
  ...
  // Gunakan useMemo untuk menghitung periode berdasarkan historyData
  const computedPeriode = useMemo(() => {
    ...
    return Math.abs(calcT) > 1000 || calcT < -1000 ? '∞' : calcT.toFixed(2);
  }, [historyData]);

  // Update state periode hanya jika hasil perhitungan berubah (tanpa setState di dalam effect yang dipicu tiap render)
  useEffect(() => {
    setPeriode(computedPeriode);
  }, [computedPeriode]);
      ...
      <div className="flex-1 grid grid-cols-2 grid-rows-2 gap-4 min-h-0">
        ...
        <div className="bg-black border border-gray-800 rounded relative group p-3 flex flex-col">
          ...
          <div className="flex-1 relative w-full h-full">
            <ChartLine
              ...
              data={{
                datasets: [{
                  data: filteredData.map(d => ({ x: d.time, y: d.dayaWatt })),
                  borderColor: '#3B82F6',
                  borderWidth: 2,
                  tension: 0.1
                }]
        ...
        <div className="bg-black border border-gray-800 rounded relative group p-3 flex flex-col">
          ...
          <div className="flex-1 relative w-full h-full">
            <ChartLine
              ...
              data={{
                datasets: [{
                  data: filteredData.map(d => ({ x: d.time, y: d.reaktivitas })),
                  borderColor: '#F97316',
                  borderWidth: 2,
                  tension: 0.1
                }]
            ...
            <ChartLine
              ...
              data={{
                datasets: [0,1,2,3,4,5].map(i => ({
                  data: filteredData.map(d => ({ x: d.time, y: d.prekursor[i] })),
                  borderColor: ['#EF4444', '#F59E0B', '#EAB308', '#22C55E', '#06B6D4', '#A855F7'][i],
                  borderWidth: 1.5,
                  tension: 0.1
                }))
              }}
        ...
        <div className="bg-black border border-gray-800 rounded relative group p-3 flex flex-col justify-center items-center">
          ...
          <div className="text-5xl font-mono font-bold text-white tracking-widest">
            {periode} <span className="text-xl text-gray-500">s</span>
          </div>
